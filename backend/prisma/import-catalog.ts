...
async function buildUniqueProductSlug(options: {
  sku: string;
  nsCode: string | null;
  barcode: string | null;
}) {
  const candidates = [
    slugify(options.sku),
    options.nsCode ? slugify(`${options.sku}-${options.nsCode}`) : null,
    options.barcode ? slugify(`${options.sku}-${options.barcode}`) : null,
    slugify(`${options.sku}-${shortHash(options.sku)}`),
  ...
  for (const candidate of candidates) {
    ...
    if (!existing || existing.sku === options.sku) {
      return candidate;
    }
  }

  // As a final fallback, keep extending with a deterministic suffix.
  const base = candidates[0] ?? slugify(options.sku);
  for (let index = 2; index < 25; index += 1) {
    const candidate = `${base}-${index}-${shortHash(options.sku)}`;
    const existing = await prisma.product.findUnique({
      where: { slug: candidate },
      select: { sku: true },
    });
    ...
    try {
      ...
      const product = existingBySku
        ...
        : await prisma.product.create({
            data: {
              categoryId,
              slug: await buildUniqueProductSlug({ sku, nsCode, barcode }),
              sku,
              name,
              brand: brand || null,
              brandLabel: brand || null,
