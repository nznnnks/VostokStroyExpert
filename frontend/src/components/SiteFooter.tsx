  ...
  return (
    <footer className="mt-16 border-t border-[#e8e3db] bg-[#f7f5f1] px-4 py-9 md:px-10 md:py-10 xl:px-12 xl:py-12 2xl:px-16 2xl:py-14">
      <div className="mx-auto grid max-w-[1480px] gap-8 xl:grid-cols-[1.1fr_1.2fr_1fr] xl:items-start xl:gap-10 2xl:max-w-[1860px] 2xl:gap-12">
        ...
        {/* Mobile/Tablet footer: keep "Services" directly under "Site map" regardless of "Client" column height */}
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 xl:hidden">
          <div className="min-w-0 space-y-8">
            <div className="min-w-0">
              <h3 className="text-[16px] uppercase [font-family:'Cormorant_Garamond',serif] md:text-[18px]">{columnTitles[0]}</h3>
              <div className="mt-4 space-y-3 text-[13px] uppercase tracking-[1.5px] text-[#7a7a75] md:mt-5 md:space-y-4 md:text-[14px] [font-family:Jaldi,'JetBrains_Mono',monospace]">
                {(footerColumns[0] ?? []).map((link) => (
                  <a
                    ...
                    href={link.href}
                    className="block leading-[1.45] whitespace-normal transition duration-300 ease-out hover:translate-x-1 hover:text-[#2f2f2a]"
                  >
                    {link.label}
                  </a>
                ))}
            ...
            <div className="min-w-0">
              <h3 className="text-[16px] uppercase [font-family:'Cormorant_Garamond',serif] md:text-[18px]">{columnTitles[2]}</h3>
              <div className="mt-4 space-y-3 text-[13px] uppercase tracking-[1.5px] text-[#7a7a75] md:mt-5 md:space-y-4 md:text-[14px] [font-family:Jaldi,'JetBrains_Mono',monospace]">
                {(footerColumns[2] ?? []).map((link) => (
                  <a
                    key={link.href + link.label}
                    href={link.href}
                    className="block leading-[1.45] whitespace-normal transition duration-300 ease-out hover:translate-x-1 hover:text-[#2f2f2a]"
          ...
          <div className="min-w-0">
            <h3 className="text-[16px] uppercase [font-family:'Cormorant_Garamond',serif] md:text-[18px]">{columnTitles[1]}</h3>
            <div className="mt-4 space-y-3 text-[13px] uppercase tracking-[1.5px] text-[#7a7a75] md:mt-5 md:space-y-4 md:text-[14px] [font-family:Jaldi,'JetBrains_Mono',monospace]">
        ...
        <div className="hidden xl:grid xl:grid-cols-3 xl:gap-8 xl:pt-[2px]">
          {footerColumns.map((column, index) => (
            <div key={index} className="min-w-0">
              <h3 className="text-[20px] uppercase [font-family:'Cormorant_Garamond',serif] 2xl:text-[22px]">{columnTitles[index] ?? "Раздел"}</h3>
              <div className="mt-6 space-y-6 text-[15px] uppercase tracking-[1.5px] text-[#7a7a75] 2xl:text-[17px] [font-family:Jaldi,'JetBrains_Mono',monospace]">
                {column.map((link) => (
                  <a
                    key={link.href + link.label}
                    href={link.href}
                    className="block whitespace-normal transition duration-300 ease-out hover:translate-x-1 hover:text-[#2f2f2a]"
                  >
                    {link.label}
                  </a>
                ))}
